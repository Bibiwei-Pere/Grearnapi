...
>(({ className, ...props }, ref) => (
  <AlertDialogPrimitive.Overlay
    className={cn(
      "fixed max-w-screen-2xl min-h-screen overflow-scroll mx-auto inset-0 z-50 bg-gray-500/40 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0",
      className
    )}
    {...props}
    ref={ref}
...
>(({ className, ...props }, ref) => (
  <AlertDialogPortal>
    ...
    <AlertDialogPrimitive.Content
      ref={ref}
      className={cn(
        "fixed mx-auto left-[50%] top-[50%] z-50 grid w-full max-w-lg translate-x-[-50%] translate-y-[-50%] gap-4 bg-black p-6 shadow-lg duration-200",
        "max-h-[80vh] overflow-y-auto", // Set max-height and enable vertical scrolling
...
const ErrorModal = ({ title, description, children }: Props) => {
  return (
    <AlertDialogContent className='bg-white left-[50%] top-[50%]'>
      <AlertDialogHeader className='flex-row gap-4'>
        ...
        <div>
          <AlertDialogTitle className='text-black pb-2'>{!title && "An error occured as occured!"}</AlertDialogTitle>
